                                ...
                                <div class="card list-topic-content py-5 px-2 col-12">
                                    <div class="row p-2 px-5">
                                        ...
                                        <div class="col-lg-12 text-start">
                                            <a href="{{ route('admin.detail.result.learning.activity.detail.step', ['user_id' => Crypt::encryptString($user_id_next_step), 'slug' => Crypt::encryptString($slug_next_step), 'step' => Crypt::encryptString(($next_step - 1))]) }}" id="btn-step-2" class="btn btn-information text-white">Kembali ke Sintak 2.</a>

                                            @if ( $is_enable_to_next_step )
                                            <a href="{{ route('admin.detail.result.learning.activity.detail.step', ['user_id' => Crypt::encryptString($user_id_next_step), 'slug' => Crypt::encryptString($slug_next_step), 'step' => Crypt::encryptString(($next_step + 1))]) }}" id="btn-step-3" class="btn btn-information text-white">Selanjutnya Sintak 4.</a>
                                            @endif
    ...
    input.addEventListener("change", function(){
        ...
        if(validExtensions.includes(fileType)){ //if user selected file is an image file
            ...
            fileReader.onload = ()=>{
                ...
                let videoTag = `<video width="400" controls><source src="${fileURL}" type="video/mp4"></video>`
                ...
                $('div.render-file').html(videoTag); //adding that created img tag inside dropArea container
            }
...
<script>
    @if ( $detail_step != null )
        ...
        @foreach (json_decode($value_answers) as $value)
            let videoTag = `<video width="400" controls><source src="{{ asset('assets/activity/step/') . '/' . $value->value_text }}" type="video/mp4"></video>`
            $('div.render-file').html(videoTag); //adding that created img tag inside dropArea container
        @endforeach
    @endif
</script>
