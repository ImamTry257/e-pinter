            ...
            <div class="col-lg-12 p-4">
                ...
                <div class="prolog pb-3 mb-3 px-3" style="background-color: #f3f6f9 !important;">
                    ...
                    <div class="wrapper-adjust-info pb-2">
                        <span>Mohon Kuisioner Sesi {{ ucwords(str_replace('-', ' ', $type_selected)) }} ini diisi dengan sungguh-sungguh untuk umpan balik dalam perbaikan pelayanan bagi mahasiswa dan tidak akan mempengaruhi nilai dalam perkuliahan</span>
                    </div>
                ...
                <div class="wrapper-content">
                    ...
                    <div class="title-navigation d-flex justify-content-between pb-3">
                        <div class="prev">
                            @if ( ( $current_page - 1 ) > 0 )
                            <a href="{{ route('questionnaire.type', ['type' => $type_selected, 'page' => ( $current_page - 1 )]) }}" class="btn btn-primary" title="Menuju ke halaman sebelumnya">
                        ...
                        <div class="next">
                            ...
                            <a href="{{ route('questionnaire.type', ['type' => $type_selected, 'page' => ( $current_page + 1 ) ]) }}" class="btn btn-primary" title="Menuju ke halaman selanjutnya">
                    ...
                    <div class="wrapper-questionnaire">
                        <table class="table table-bordered w-100">
                            ...
                            <tr class="@if( array_key_exists($data->number, $list_answer) ) bg-filled @else bg-unfilled @endif">
                                ...
                                @foreach ( $list_option_type as $index => $option_type )
                                <td align="center">
                                    <input
                                        type="radio"
                                        class="{{ $option_type }}"
...
<script>
    ...
    function saveAnswer(e){
        ...
        formData.append('type_selected', '{{ $type_selected }}')

        $.ajax({
            type: "POST", // send ajax with post
            url: "{{ route('questionnaire.store.type') }}",
            dataType: 'json',
            data: formData,
            timeout: 30000,
            cache: false,
