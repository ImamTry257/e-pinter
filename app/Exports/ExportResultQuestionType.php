...
class ExportResultQuestionType implements FromView, WithEvents, WithStyles, WithColumnWidths
{
    public $type;
        ...
        if ( count ( $get_detail_user ) != 0 ) :
            foreach ( $get_detail_user as $key => $val ) :
                ...
                foreach ( $data_user_answer as $dd ) :
                    $data_selected [$dd->number] = $dd->score;
                endforeach;
                ...
                $d_user_answer = [];
                foreach ( $number_question as $number ) :
                    if ( in_array( $number, $number_selected ) ) :
                        $d_user_answer[] = [
                            'number' => $number,
                            'score' => $data_selected[$number]
                        ];
                    else :
                        $d_user_answer[] = [
                            'number' => $number,
