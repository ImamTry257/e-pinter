...
class QuestionController extends Controller
{
    ...
    public function introduction()
    {
        ...
        # jika tidak ada param current duration di URL, maka user baru akan start
        $data['question_is_ready'] = true;
        if ( $current_duration == 0 ) :
            $data['question_is_ready'] = false;
        endif;

        if ( $is_before_session_exist ) :
            Session::put('current_duration_pre_test', $current_duration);
            Session::put('current_duration_post_test', $current_duration);
            Session::save();
        else :
            if ( Session::get('current_duration') == null ) :
                Session::put('current_duration_pre_test', $current_duration);
        ...
        # dd($data);
        return view('front.page.question.introduction', $data);
    }
    ...
    public function index_type($type, $questionNo)
    {
        ...
        # dd($data, $type);
            ...
            if ( $type == 'pre-test' ) :
                if ( Session::get('current_duration_pre_test') == null ) :
                    Session::put('current_duration_pre_test', $current_duration);
                endif;
            else :
                if ( Session::get('current_duration_post_test') == null ) :
                    Session::put('current_duration_post_test', $current_duration);
                endif;
            endif ;
        endif ;
        ...
        # get data selected question and question of user answer
        $data['question_selected'] = DB::table('question_master as qm')
                            ->join('question_answer_key as qak', 'qak.question_master_id', '=', 'qm.id')
                            ->where('qm.number', '=', $question_no)
                            ->first();

        $data['user_answer'] = DB::table('question_master as qm')
                            ->leftJoin('question_answer_user as qau', 'qau.question_master_id', '=', 'qm.id')
                            ->where('qm.number', '=', $question_no)
                            ->where('qau.type', '=', $type)
        ...
        return view('front.page.question.index_type', $data);
    }

    public function store_type(Request $request)
    {
        # dd($request->all(), $request->get('type'));
        ...
        return redirect(route('question.type', ['type' => $request->type_selected, 'questionNo' => Crypt::encryptString($question_after_action)]). '-' . $request->countdown_str);
    }
}
