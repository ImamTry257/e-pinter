...
class QuestionnaireController extends Controller
{
    ...
    public function introduction()
    {
        ...
        return view('front.page.questionnaire.introduction', $data);
    }

    public function start($type, $page)
    {
        $user_id = Auth::user()->id;
        ...
        $data['data_questionniare'] = DB::table('questionniare_master as qm')
            ->where('qm.page', '=', $page)
            ->orderBy('qm.number', 'asc')
            ->get(['qm.number', 'qm.number_string', 'qm.description', 'qm.id', 'qm.statement_type']);
        ...
        # list data option
        $data['list_option_type'] = ['SI', 'SR', 'KD', 'TP'];
        $data['list_statement_type'] = ['positive', 'negative'];
        $data['list_positive_value'] = [4, 3, 2, 1];
        $data['list_negative_value'] = [1, 2, 3, 4];

        # type selected
        $data['type_selected'] = $type;
        # dd($data);

        return view('front.page.questionnaire.index_type', $data);
    }
